...
func NewUnifiedCollector(client *rpc.Client, cfg *config.Chain, prometheusURL string) *UnifiedCollector {
	...
	return &UnifiedCollector{
		...
		validatorRank:        prometheus.NewDesc("cosmos_validator_rank", "Validator rank", []string{"chain_id", "address", "moniker"}, nil),
		...
		validatorJailedDesc:  prometheus.NewDesc("cosmos_validator_jailed", "Validator jailed status", []string{"chain_id", "address", "moniker"}, nil),
		...
		validatorStatusDesc:      prometheus.NewDesc("cosmos_validators_status", "Validator status", []string{"chain_id", "address", "moniker", "status"}, nil),
...
func (c *UnifiedCollector) collectTenderdutyMetrics(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		...
		if height, err := strconv.ParseFloat(status.Result.SyncInfo.LatestBlockHeight, 64); err == nil {
			...
			c.logger.Debug("Successfully parsed block height", "height", height)
		} else {
...
func (c *UnifiedCollector) collectGeneralMetrics(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		...
		ch <- prometheus.MustNewConstMetric(c.bondedTokens, prometheus.GaugeValue, bondedTokens, c.cfg.ChainID)
		ch <- prometheus.MustNewConstMetric(c.notBondedTokens, prometheus.GaugeValue, notBondedTokens, c.cfg.ChainID)
	}
	...
	if err != nil {
		...
		for _, coin := range communityPool.Pool {
			...
			ch <- prometheus.MustNewConstMetric(c.communityPool, prometheus.GaugeValue, amount, c.cfg.ChainID, coin.Denom)
		}
	}
	...
	if err != nil {
		...
		for _, coin := range supply.Supply {
			amount, _ := strconv.ParseFloat(coin.Amount, 64)
			ch <- prometheus.MustNewConstMetric(c.supplyTotal, prometheus.GaugeValue, amount, c.cfg.ChainID, coin.Denom)
		}
	}
		...
		if err != nil {
			...
			for _, coin := range balance.Balances {
				if amount, err := strconv.ParseFloat(coin.Amount, 64); err == nil {
					...
					ch <- prometheus.MustNewConstMetric(c.walletBalance, prometheus.GaugeValue, displayAmount, c.cfg.ChainID, wallet.Address, coin.Denom)
				}
			}
		}
		...
		if err != nil {
			...
			for _, delegation := range delegations.DelegationResponses {
				if amount, err := strconv.ParseFloat(delegation.Balance.Amount, 64); err == nil {
					displayAmount := amount / math.Pow10(c.cfg.TokenDecimals)
					ch <- prometheus.MustNewConstMetric(c.walletDelegations, prometheus.GaugeValue, displayAmount, c.cfg.ChainID, wallet.Address, delegation.Balance.Denom, delegation.Delegation.ValidatorAddress)
				}
			}
		}
		...
		if err != nil {
			...
			for _, reward := range rewards.Rewards {
				for _, coin := range reward.Reward {
					if amount, err := strconv.ParseFloat(coin.Amount, 64); err == nil {
						displayAmount := amount / math.Pow10(c.cfg.TokenDecimals)
						ch <- prometheus.MustNewConstMetric(c.walletRewards, prometheus.GaugeValue, displayAmount, c.cfg.ChainID, wallet.Address, coin.Denom, reward.ValidatorAddress)
					}
				}
			}
		}
...
func (c *UnifiedCollector) collectValidatorsMetrics(ch chan<- prometheus.Metric) error {
	...
	// Active and inactive validators count
	...
	// config.yml에 정의된 특정 벨리데이터들만 개별 메트릭 수집
	...
	for _, validator := range validators.Validators {
		...
		if tokens, err := strconv.ParseFloat(validator.Tokens, 64); err == nil {
			displayTokens := tokens / math.Pow10(c.cfg.TokenDecimals)
		...
		if err != nil {
			...
			for _, coin := range commission.Commission.Commission {
				if amount, err := strconv.ParseFloat(coin.Amount, 64); err == nil {
					displayAmount := amount / math.Pow10(c.cfg.TokenDecimals)
					ch <- prometheus.MustNewConstMetric(c.validatorCommission, prometheus.GaugeValue, displayAmount, c.cfg.ChainID, validator.OperatorAddress, validator.Description.Moniker, coin.Denom)
				}
			}
		}
		...
		if err != nil {
			...
			for _, coin := range rewards.Rewards.Rewards {
				if amount, err := strconv.ParseFloat(coin.Amount, 64); err == nil {
					displayAmount := amount / math.Pow10(c.cfg.TokenDecimals)
					ch <- prometheus.MustNewConstMetric(c.validatorRewards, prometheus.GaugeValue, displayAmount, c.cfg.ChainID, validator.OperatorAddress, validator.Description.Moniker, coin.Denom)
				}
			}
		}
		...
		if validator.ConsensusAddress != "" {
			signingInfo, err := c.client.GetSigningInfos()
			if err != nil {
				c.logger.Debug("Failed to get signing infos", "error", err)
			} else {
				for _, info := range signingInfo.Info {
					if info.Address == validator.ConsensusAddress {
						if missedBlocks, err := strconv.ParseFloat(info.MissedBlocksCounter, 64); err == nil {
							ch <- prometheus.MustNewConstMetric(c.validatorMissedBlocks, prometheus.GaugeValue, missedBlocks, c.cfg.ChainID, validator.OperatorAddress, validator.Description.Moniker)
						}
						break
					}
				}
		...
		ch <- prometheus.MustNewConstMetric(c.validatorStatus, prometheus.GaugeValue, float64(statusCode), c.cfg.ChainID, validator.OperatorAddress, validator.Description.Moniker)
		...
		if delegatorShares, err := strconv.ParseFloat(validator.DelegatorShares, 64); err == nil {
			displayShares := delegatorShares / math.Pow10(c.cfg.TokenDecimals)
			ch <- prometheus.MustNewConstMetric(c.validatorDelegatorShares, prometheus.GaugeValue, displayShares, c.cfg.ChainID, validator.OperatorAddress, validator.Description.Moniker)
		}

		// Validator status (cosmos_validators_status)
		ch <- prometheus.MustNewConstMetric(c.validatorStatusDesc, prometheus.GaugeValue, 1, c.cfg.ChainID, validator.OperatorAddress, validator.Description.Moniker, validator.Status)
	}
...
func (c *UnifiedCollector) analyzeBlock(height int) {
	...
	for validatorAddr, state := range c.validatorStates {
		...
		if state.consensusAddress == "" {
			c.logger.Warn("No consensus address for validator", "validator", validatorAddr)
			continue
		}
